...
// 返回行数
int InboundTableModel::rowCount(const QModelIndex& parent) const
{
    if (parent.isValid())
        return 0;
...
// 返回列数
int InboundTableModel::columnCount(const QModelIndex& parent) const
{
    if (parent.isValid())
        return 0;
...
// 返回索引的数据
QVariant InboundTableModel::data(const QModelIndex& index, int role) const
{
    if (!index.isValid())
        return {};
    if (index.row() < 0 || index.row() >= page_.items.size())
        return {};
    const InboundOrderListItemDto& order = page_.items[index.row()];
    if (role == Qt::DisplayRole || role == Qt::EditRole) {
        switch (static_cast<Column>(index.column())) {
        case Column::OrderNoColumn:
            return order.orderNo;
        case Column::SupplierColumn:
...
// 设置展示的入库订单信息
void InboundTableModel::setPage(const PageResult<InboundOrderListItemDto>& page)
{
    beginResetModel();
    this->page_ = page;
    endResetModel();
...
// 返回表头数据
QVariant InboundTableModel::headerData(int section, Qt::Orientation orientation, int role) const
{
    if (orientation == Qt::Horizontal && role == Qt::DisplayRole) {
        switch (static_cast<Column>(section)) {
        case Column::OrderNoColumn:
            return QStringLiteral("入库订单编号");
        case Column::SupplierColumn:
            return QStringLiteral("供应商");
        case Column::StatusColumn:
...
// 获取指定行的入库订单
InboundOrderListItemDto InboundTableModel::itemAt(int row) const
{
...
// 获取对应行的入库订单ID
quint32 InboundTableModel::orderIdAt(int row) const
{
    if (row >= 0 && row < page_.items.size())
...
// 获取总数
int InboundTableModel::total() const noexcept
{
    return page_.total;
}
// 获取对应行的入库订单操作人ID
quint32 InboundTableModel::operatorIdAt(int row) const
{
    if (row >= 0 && row < page_.items.size())
...
// 获取对应行的入库订单仓库ID
quint32 InboundTableModel::warehouseIdAt(int row) const
{
    if (row >= 0 && row < page_.items.size())
...
// 获取当前页码
int InboundTableModel::page() const noexcept
{
    return page_.page;
}
// 获取每页的大小
