...
public:
    ...
    using WarehouseListCallback = std::function<void(const WarehouseListResult&)>; // 仓库列出回调
    ...
    // 列出所有仓库(是否只展示活跃)
    virtual void listWarehouses(bool activeOnly, QObject* owner, const WarehouseListCallback callback) = 0;
};